...
<div>
  <div class="detail" v-if="loading === 1">
    <div class="detail-title df-between">
      <div class="df-between" @click="hasHistory() ? $router.go(-1) : $router.push('/')">
    ...
    <h2 class="fz-md c-main">
      ...
      <span v-text="' ' + result[`${mode}Name`] + '資訊'"></span>
    </h2>
    <div class="detail-info bdrs-sm">
      ...
      <p v-if="result.Address">
        ...
        <span v-text="` ${result[`${mode}Name`]}地點：`"></span>
      ...
      <p v-if="result.Class || result.Class1 || result.Class2 || result.Class3">
        ...
        <span v-text="` ${result[`${mode}Name`]}標籤：`"></span>
        <router-link
    ...
    <h2 class="fz-md c-main">
      ...
      <span v-text="' ' + result[`${mode}Name`] + '介紹'"></span>
    </h2>
    <pre class="detail-pre" v-text="result.DescriptionDetail || result.Description"></pre>
    <h2 class="fz-md c-main"><i class="icofont-bus"></i> 交通方式</h2>
    ...
    <div class="bdrs-sm">
      <iframe
        ...
        loading="lazy"
        v-if="mode === 'Activity'"
        :src="`https://maps.google.com/maps?q=${result.Position.PositionLat},${result.Position.PositionLon}&hl=zh-TW&z=16&ampoutput=embed`"
      >
      </iframe>
      <iframe
        width="100%"
        ...
        loading="lazy"
        v-else
        :src="`https://maps.google.com/maps?q=${result[`${mode}Name`]}+${result[`${mode}Name`].split('').join('+')}&hl=zh-TW&z=16&ampoutput=embed`"
      >
      </iframe>
    </div>
    <div class="space"></div>
    <div class="near">
      <div v-for="item in Object.keys(modeLib)" :key="item">
        <div class="df-between">
          <h2 class="fz-md"><i class="icofont-beach"></i> 查看鄰近的{{modeLib[item]}}</h2>
          <router-link :to="`/${item}/${result.Position.PositionLat}/${result.Position.PositionLon}/`" class="detail-more">More</router-link>
        </div>
        <Recommend
          :recMode="item"
...
<script>
import { getDetail, getMode } from '../api.js'
...
export default {
  ...
  methods: {
    ...
    loadData () {
      if (!this.$route.params.ID) this.$route.replace({ name: 'Home' })
      // getDetail(this.$route.params.ID)
      //   .then((data) => {
      //     if (!data.ID) throw new Error()
      //     this.result = data
      //     this.SET_MODE(getMode(this.result.ID, true))
      //     document.title = this.result.Name + ' - Travel Guide'
      //     setTimeout(() => (this.loading = 1), 500)
      //     window.scrollTo({ top: 0, behavior: 'smooth' })
      //   })
      //   .catch(() => (this.loading = -1))

      this.result = getDetail(this.$route.params.ID)
      console.log(this.result)
      this.SET_MODE(getMode(this.result[`${this.mode}ID`], true))
      document.title = this.result.Name + ' - Travel Guide'
      setTimeout(() => (this.loading = 1), 500)
      window.scrollTo({ top: 0, behavior: 'smooth' })
    }
